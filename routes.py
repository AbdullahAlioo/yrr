...
@app.route('/chat')
def chat():
    # Check if user is in session, if not provide default values for template
    if 'user_id' in session:
        user = User.query.get(session['user_id'])
        if user:
            return render_template('chat.html', user=user)

    # For localStorage mode, provide a default user object for template rendering
    default_user = {
        'name': 'User',
        'user_id': 'temp',
        'unique_id': 'TEMP',
    ...
    return render_template('chat.html', user=default_user)
...
def api_mark_seen():
    ...
    try:
        ...
        if not conversation_id:
            return jsonify({'success': False, 'message': 'Conversation ID required'})
    ...
    try:
        ...
        if conv_type == 'private':
            ...
            if not target_user_id:
                return jsonify({'success': False, 'message': 'Target user ID required'})
            ...
            existing_conv = db.session.query(Conversation).join(
                ...
                Conversation.type == 'private',
                ConversationParticipant.user_id.in_([user_id, target_user_id])
            ).group_by(Conversation.id).having(
                db.func.count(ConversationParticipant.user_id) == 2
            ).first()
            ...
            if existing_conv:
                participants = ConversationParticipant.query.filter_by(conversation_id=existing_conv.id).all()
                participant_ids = [p.user_id for p in participants]
                if set(participant_ids) == {user_id, target_user_id}:
                    return jsonify({'success': True, 'conversation_id': existing_conv.id})
            ...
            conversation = Conversation(
                name=f"Private chat with {target_user.name}",
                type='private',
                created_by=user_id
            )
            ...
            # Add participants
            participant1 = ConversationParticipant(conversation_id=conversation.id, user_id=user_id)
            participant2 = ConversationParticipant(conversation_id=conversation.id, user_id=target_user_id)
            ...
            if len(participant_ids) < 1 or len(participant_ids) > 9:
                return jsonify({'success': False, 'message': 'Group must have 2-10 members (including you)'})
            ...
            for participant_id in participant_ids:
                if participant_id != user_id:
                    participant = ConversationParticipant(conversation_id=conversation.id, user_id=participant_id)
                    db.session.add(participant)
