...
async function loadConversations() {
    try {
        ...
        const cachedConversations = getConversationsFromStorage();
        if (cachedConversations.length > 0) {
            conversations = cachedConversations;
            renderConversations();
        }
        
        // Then try to load from API (fallback for server-based conversations)
        try {
...
async function loadMessages(conversationId) {
    try {
        ...
        const cachedMessages = getMessagesFromStorage(conversationId);
        ...
        try {
            ...
            if (response.success && response.messages) {
                ...
                saveMessagesToStorage(conversationId, messages[conversationId]);
                renderMessages(conversationId);
            }
        } catch (apiError) {
            console.warn('API not available, using localStorage only');
...
async function handleSendMessage(event) {
    ...
    // Add to local state and storage immediately
    if (!messages[window.currentConversation]) {
        messages[window.currentConversation] = [];
    }
    messages[window.currentConversation].push(newMessage);
    saveMessagesToStorage(window.currentConversation, messages[window.currentConversation]);
    
    // Render instantly
    renderMessages(window.currentConversation);
    
    // Try to send via API in background
    try {
        ...
        if (response.success) {
            ...
            const tempIndex = messages[window.currentConversation].findIndex(msg => msg.id === newMessage.id);
            if (tempIndex !== -1) {
                messages[window.currentConversation][tempIndex] = response.message;
                saveMessagesToStorage(window.currentConversation, messages[window.currentConversation]);
                renderMessages(window.currentConversation);
            }
        }
    } catch (error) {
        console.warn('API not available, message saved locally only');
    }
}
...
function saveMessagesToStorage(conversationId, messageList) {
    const storageKey = `whatsapp_messages_${conversationId}`;
